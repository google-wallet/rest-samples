...
class DemoFlight
{
  ...
  public string CreateFlightClass(string issuerId, string classSuffix)
  {
  ...
  /// <param name="issuerId">The issuer ID being used for this request.</param>
  /// <param name="classSuffix">Developer-defined unique ID for this pass class.</param>
  public async void BatchCreateFlightObjects(string issuerId, string classSuffix)
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      ...
      // See link below for more information on required properties
      // https://developers.google.com/wallet/tickets/boarding-passes/rest/v1/flightobject
      FlightObject batchFlightObject = new FlightObject
      {
        Id = objectId,
        ClassId = $"{issuerId}.{classSuffix}",
        State = "ACTIVE",
      ...
      data += JsonConvert.SerializeObject(batchFlightObject) + "\n\n";
    }
    data += "--batch_createobjectbatch--";
