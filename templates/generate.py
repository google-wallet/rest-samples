...
payloads["generic"]["$object_payload"].update(
  {
    ...
    "cardTitle": {
      "defaultValue": {"language": "en-US", "value": "Testing Generic Title"}
    },
    "header": {
      "defaultValue": {"language": "en-US", "value": "Testing Generic Header"}
    },
    "subheader": {
...
lang_config = {
  "java": {
    ...
    + s.replace('"', '\\"').replace("\n", '"\n      + "')
    + '"',
    "filename": lambda s: f"src/main/java/Demo{s.title()}.java",
  },
  "python": {
    "ext": "py",
  ...
  "dotnet": {
    ...
    "filename": lambda s: f"Demo{s.title()}.cs",
    "indent": 2,
  },
  "http": {
    "ext": "http",
...
for lang, config in lang_config.items():
  ...
  for object_type, content in payloads.items():
    ...
    with open(
        path("..", lang, config["filename"](object_type)), "w", encoding="utf-8"
    ) as f:
      f.write(output)
